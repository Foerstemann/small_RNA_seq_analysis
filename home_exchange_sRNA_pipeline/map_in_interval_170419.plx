...
while (<LIBFILE>) 
{
	...
	if (defined $next_name and $sequence_counter > 1) {
		...
		$name_hash{$seq_name[$sequence_counter-1]} = $sequence_counter-1;
...
close(INFILE);
print OUTFILE "total number of sequences in FASTA reference file: \t", $sequence_counter,"\n";
